...
def segmentate(volume, roi_coords=(100, 600, 100, 600)):
    ...
    for i in tqdm(range(z), desc='Segmentacja strukturalna'):
        ...
        try:
            ...
            potential = (probs > 0.5).astype('uint8')
...
def extract_main_bone_v1(bone_mask_3d, roi_coords=(100, 600, 100, 600), min_volume=100):
    """
    Wersja 1: Filtrowanie małych fragmentów zamiast agresywnej erozji
    - Mniej zniszczeń struktury głównej
    - Usuwa artefakty i szumy
    """
    r_s, r_e, c_s, c_e = roi_coords
    clean_mask = np.zeros_like(bone_mask_3d)
    mask_roi = (bone_mask_3d[:, r_s:r_e, c_s:c_e] > 0)
    
    if not np.any(mask_roi):
        return clean_mask
    
    # Label wszystkich komponentów
    labels = measure.label(mask_roi, connectivity=3)
    props = measure.regionprops(labels)
    
    if not props:
        return clean_mask
    
    # Filtruj fragmenty: zachowaj tylko te znaczące
    significant_labels = set()
    for prop in props:
        if prop.area >= min_volume:  # Dostosuj min_volume do swoich danych
            significant_labels.add(prop.label)
    
    # Utwórz maskę ze znaczącymi fragmentami
    significant_mask = np.zeros_like(labels, dtype=bool)
    for label in significant_labels:
        significant_mask |= (labels == label)
    
    final_bone_roi = significant_mask.astype(np.uint8) * 255
...
if __name__ == "__main__":
    ...
    main_bone_shell = extract_main_bone_v1(initial_mask, roi_coords=COORDS, min_volume=100)
    #full_bone_solid = fill_bone_volume(main_bone_shell, closing_radius=40)
    view(full_vol, main_bone_shell, main_bone_shell)